...
from patternly._utils import RANDOM_NAME, UnionFind
...
class AnomalyDetection:
    ...
    def __calculate_cluster_PFSAs(self):
        ...
        for i in range(self.n_clusters):
            ...
            if PFSA_found:
                ...
                raise ValueError(f"Unable to find PFSA for cluster {i + 1}/{self.n_clusters}")
    ...
    def __reduce_clusters(self):
        ...
        for i in range(self.n_clusters):
            ...
            # list of cluster PFSAs sorted in descending order
        ...
        graph = UnionFind(self.n_clusters)
        for i in range(self.n_clusters):
            ...
            if second_best_match != i and all_cluster_likelihoods[i][second_best_match] > 2 * (1 / self.n_clusters): # arbitray threshold
                graph.union(i, second_best_match, ranks=(all_cluster_likelihoods[i] + all_cluster_likelihoods[second_best_match]))
        graph = graph.compress_all()
        new_n_clusters = graph.n_components
